...
grant select, insert, update, delete on table public.video_sources to authenticated;
grant select, insert, update, delete on table public.video_snapshots to authenticated;
...
grant select, insert, update, delete on table public.generated_artifacts to authenticated;
grant select, insert, update, delete on table public.knowledge_jobs to authenticated;
grant select, insert, update, delete on table public.expression_senses to authenticated;
grant select, insert, update, delete on table public.expression_occurrences to authenticated;
grant select, insert, update, delete on table public.user_expressions to authenticated;
...
grant select, insert, update, delete on table public.mastery_events to authenticated;
...
create policy generated_artifacts_select_own on public.generated_artifacts
  for select to authenticated using ((select auth.uid()) = user_id);
create policy generated_artifacts_insert_own on public.generated_artifacts
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy generated_artifacts_update_own on public.generated_artifacts
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy generated_artifacts_delete_own on public.generated_artifacts
  for delete to authenticated using ((select auth.uid()) = user_id);

create policy knowledge_jobs_select_own on public.knowledge_jobs
  for select to authenticated using ((select auth.uid()) = user_id);
create policy knowledge_jobs_insert_own on public.knowledge_jobs
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy knowledge_jobs_update_own on public.knowledge_jobs
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy knowledge_jobs_delete_own on public.knowledge_jobs
  for delete to authenticated using ((select auth.uid()) = user_id);

create policy expression_senses_select_own on public.expression_senses
  for select to authenticated using ((select auth.uid()) = user_id);
create policy expression_senses_insert_own on public.expression_senses
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy expression_senses_update_own on public.expression_senses
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy expression_senses_delete_own on public.expression_senses
  for delete to authenticated using ((select auth.uid()) = user_id);

create policy expression_occurrences_select_own on public.expression_occurrences
  for select to authenticated using ((select auth.uid()) = user_id);
create policy expression_occurrences_insert_own on public.expression_occurrences
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy expression_occurrences_update_own on public.expression_occurrences
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy expression_occurrences_delete_own on public.expression_occurrences
  for delete to authenticated using ((select auth.uid()) = user_id);

create policy user_expressions_select_own on public.user_expressions
  for select to authenticated using ((select auth.uid()) = user_id);
create policy user_expressions_insert_own on public.user_expressions
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy user_expressions_update_own on public.user_expressions
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy user_expressions_delete_own on public.user_expressions
  for delete to authenticated using ((select auth.uid()) = user_id);

create policy practice_tasks_select_own on public.practice_tasks
  for select to authenticated using ((select auth.uid()) = user_id);
create policy practice_tasks_insert_own on public.practice_tasks
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy practice_tasks_update_own on public.practice_tasks
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy practice_tasks_delete_own on public.practice_tasks
  for delete to authenticated using ((select auth.uid()) = user_id);
...
create policy mastery_events_select_own on public.mastery_events
  for select to authenticated using ((select auth.uid()) = user_id);
create policy mastery_events_insert_own on public.mastery_events
  for insert to authenticated with check ((select auth.uid()) = user_id);

create policy review_tasks_select_own on public.review_tasks
  for select to authenticated using ((select auth.uid()) = user_id);
create policy review_tasks_insert_own on public.review_tasks
  for insert to authenticated with check ((select auth.uid()) = user_id);
create policy review_tasks_update_own on public.review_tasks
  for update to authenticated using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
create policy review_tasks_delete_own on public.review_tasks
  for delete to authenticated using ((select auth.uid()) = user_id);
