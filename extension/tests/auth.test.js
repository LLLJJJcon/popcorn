...
const getAuth = () => import("../auth.js");

function response(body, ok = true, status = ok ? 200 : 400) {
  return { ok, status, json: async () => body };
}

function createChrome({ onLaunch } = {}) {
  const local = {};
  const session = {};
  const calls = [];
    ...
    chrome: {
      ...
      identity: {
        getRedirectURL(pathname) {
          ...
          return `${REDIRECT_ORIGIN}/supabase`;
        },
...
test("interactive sign-in is click-gated, stores PKCE before launch, and stores only a session", async () => {
  ...
  await assert.rejects(client.handleMessage({ source: "content-script", type: "get-access-token" }), /untrusted/i);
});
