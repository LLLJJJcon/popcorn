...
type ConfigWithLegacyOrigin = ConfigRow & { model_gateway_origins: OriginRow | null };

function configRecord(row: ConfigWithLegacyOrigin): ModelGatewayConfigRecord {
  const canonicalOrigin = row.canonical_origin ?? row.model_gateway_origins?.canonical_origin;
  const basePath = row.base_path ?? row.model_gateway_origins?.base_path;
  if (!canonicalOrigin || basePath === undefined || basePath === null) {
    throw new Error("model gateway settings unavailable");
  }
  return {
    id: row.id,
    userId: row.user_id,
    ...
    consentedBaseUrl: row.consented_origin,
...
const configColumns = `id,user_id,display_name,origin_id,canonical_origin,base_path,adapter_kind,model,revision,config_fingerprint,state,consent_policy_version,consented_origin,consented_at,revoked_at,created_at,updated_at,model_gateway_origins(${originColumns})`;
...
export function createModelGatewaySettingsRepository(
  ...
  return {
    async listConfigs(userId) {
      ...
      for (const row of result.data as unknown as ConfigWithLegacyOrigin[]) {
    ...
    async findConfig(userId, configId) {
      ...
      const record = configRecord(result.data as unknown as ConfigWithLegacyOrigin);
